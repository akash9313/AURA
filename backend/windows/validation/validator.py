"""
Desktop Capability Validator Master Controller.
Validates desktop capabilities through the complete execution pipeline across test missions.
...
class DesktopCapabilityValidator:
    """
    Master Desktop Capability Validation Engine Controller.
    """
    ...
    def __init__(
        ...
        logger.info("DesktopCapabilityValidator initialized")
    ...
    async def validate_all_missions(
        ...
        """
        Execute and validate all desktop test missions through pipeline.

        Args:
            missions: Optional list of test missions (uses default test missions if None).

        Returns:
            Aggregated DesktopValidationMetrics report object.
        """
        test_missions = missions or get_default_desktop_test_missions()
        logger.info(f"Starting desktop validation suite for {len(test_missions)} test missions...")
