"""
Production-Grade Workflow Execution Engine Master Controller.
Executes validated task graphs generated by the AI Planner.
Contains NO planning logic. All execution passes through Capability Registry and Interaction Engine.
"""
...
class WorkflowExecutor:
    """
    Workflow Execution Engine Master Controller.
    Executes validated DAG task graphs with parallel scheduling, timeouts, cancellation, checkpoints, and retries.
    """
    ...
    def __init__(
        ...
        logger.info(f"WorkflowExecutor initialized for workflow '{self.workflow_id}'")

    async def execute_graph(self, task_graph_engine: TaskGraphEngine) -> WorkflowExecutionResult:
        """
        Execute validated TaskGraphEngine task graph.

        Args:
            task_graph_engine: TaskGraphEngine containing validated DAG nodes.

        Returns:
            WorkflowExecutionResult object.
        """
        start_time = time.time()
        logger.info(f"Executing workflow '{self.workflow_id}' with {len(task_graph_engine.nodes)} tasks...")
        ...
        for stage in stages:
            ...
            # Auto-checkpoint after stage completion
        ...
        logger.info(f"Workflow '{self.workflow_id}' completed successfully in {duration_ms}ms")
        return res

    async def _execute_single_task_with_retry(self, task_id: str) -> bool:
        """Execute task with retries and timeout boundaries."""
        ...
        while retries <= max_retries:
            ...
            try:
                # Wrap with timeout manager and scheduler semaphore
            ...
            if retries <= max_retries:
                ...
                logger.warning(f"Retrying task '{task_id}' (Attempt {retries}/{max_retries})...")
