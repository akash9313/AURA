...
Handles template generation, dependency installation, VS Code launch, and development server execution.
"""
...
class NextJsScaffolder:
    """
    Scaffolds Next.js project directory, launches VS Code, and executes dev server.
    """
    ...
    def scaffold_project(self, params: NextJsProjectParams, target_path: str) -> Dict[str, Any]:
        """
        Scaffold Next.js project directory and files.

        Args:
            params: NextJsProjectParams object.
            target_path: Full target folder path.

        Returns:
            Dictionary containing scaffolding details.
        """
        logger.info(f"Scaffolding Next.js project '{params.project_name}' at '{target_path}'...")
        ...
        # 2. Generate source directory & page
...
"""
        ...
        # 3. Generate node_modules stub for offline / mock fallback validation
        ...
        logger.info(f"Successfully scaffolded Next.js project structure at '{target_path}'.")
        return {"project_path": target_path, "package_json": True, "node_modules": True}

    def open_in_vscode(self, target_path: str) -> bool:
        """Launch VS Code for target project directory."""
        code_bin = shutil.which(self.config.vscode_executable)
        if not code_bin:
            logger.warning(f"VS Code executable '{self.config.vscode_executable}' not found on PATH. Skipping VS Code launch.")
            return False

        try:
            subprocess.Popen([code_bin, target_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            logger.info(f"Opened project in VS Code: '{target_path}'")
            return True
        except Exception as e:
            logger.warning(f"Failed to launch VS Code for '{target_path}': {e}")
            return False

    async def start_dev_server(self, target_path: str) -> asyncio.subprocess.Process:
        """Launch development server subprocess in non-blocking daemon mode."""
        logger.info(f"Starting Next.js dev server in '{target_path}' on port {self.config.localhost_port}...")
