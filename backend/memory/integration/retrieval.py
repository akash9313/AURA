"""
Mission Retrieval Engine.
Supports operational experience search across multiple axes:
- Mission ID
- Mission Type
- Capability
- Tags
...
class MissionRetrievalEngine:
    """
    Multi-axis retrieval engine for OperationalMissionRecord memory nodes.
    """
    ...
    def search_similar_missions(self, goal_query: str, top_k: int = 5) -> List[MissionSearchResult]:
        """
        Search operational experience by semantic goal similarity.
        Handles embedding generation failures by falling back to term matching.
        """
        records = self.persistence.list_all_records()
    ...
    def get_by_failure_pattern(self, failure_keyword: str) -> List[OperationalMissionRecord]:
        """Retrieve by Failure Pattern keyword."""
        records = self.persistence.list_all_records()
        matching = []
        for r in records:
            if r.status == "failed":
