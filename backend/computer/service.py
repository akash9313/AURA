"""
Computer Service.
Central entry point for desktop automation across the AURA AI Operating System.
Manages desktop applications, windows, input devices, clipboard, explorer, and UI automation.
Exposes platform-independent interfaces so the rest of AURA never calls OS Win32 APIs directly.
"""
...
class ComputerService(Service):
    ...
    def is_healthy(self) -> bool:
        ...
        return self.manager.state == ComputerState.READY and self.manager.get_health_status().healthy if hasattr(self.manager.get_health_status(), "healthy") else (self.manager.state == ComputerState.READY)
