...
Top-level AURA service integrating the AURAApplicationManager into the kernel framework.
Exposes platform-independent interfaces for launching, finding, monitoring, closing, and restarting desktop applications.
"""
...
class ApplicationManagerService(Service):
    """
    Service wrapper exposing Application Manager operations to AURA Runtime and Workflow Engine.
    """
    ...
    def __init__(
        ...
        logger.info("ApplicationManagerService initialized")
    ...
    async def launch_app(
        ...
        """Launch desktop application by executable path or name."""
    ...
    def find_app(self, name_or_id: str) -> Optional[AURAApplication]:
        """Find application by app_id or executable name."""
    ...
    async def wait_until_ready(self, app_id: str, timeout_ms: Optional[float] = None) -> bool:
        """Wait until application readiness check passes."""
    ...
    def start(self) -> None:
        logger.info("Starting ApplicationManagerService...")
        self.manager.monitor.start_monitoring()

    def stop(self) -> None:
        logger.info("Stopping ApplicationManagerService...")
