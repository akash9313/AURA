...
class SafetySystem:
    ...
    def evaluate_action(self, action: ActionType, details: str = "") -> Tuple[SafetyLevel, bool]:
        """
        Evaluate action safety level and check if user confirmation is required.

        Args:
            action (ActionType): Target desktop action.
            details (str): Context details.

        Returns:
            Tuple[SafetyLevel, bool]: (Safety level, Is allowed to execute)
        """
        ...
        if safety_level in (SafetyLevel.SENSITIVE, SafetyLevel.DANGEROUS):
            ...
            # Default policy: allow sensitive if no hook set, but log policy warning
